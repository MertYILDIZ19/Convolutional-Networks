...
class VggModel(nn.Module):
    def __init__(self, n_class, fine_tune, pretrained=True):
        ...
        # Get the vgg11_bn model with avg pooling and fully connected layers
        vgg11_bn_all = models.vgg11_bn(pretrained)
        self.model = nn.Sequential()
        self.vgg11_bn = list(vgg11_bn_all.children())[0]
        self.classifier = nn.Sequential(nn.Linear(layer_config[0], layer_config[1]), nn.BatchNorm1d(layer_config[1]),
                                        nn.ReLU(), nn.Linear(layer_config[1], num_classes))
...
if fine_tune:
    ...
    for param in model.vgg11_bn.parameters():
        param.requires_grad = False
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        # We have set the patience as 30 instead of 7 to keep the model running for 30 epocs.
